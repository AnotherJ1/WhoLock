...
- [ ] 1. 初始化 Rust 工程与基础设施
  ...
  - [ ] 1.4 实现 `logging.rs` 骨架与启动初始化
    ...
    - _Depends on: 1.1_
  - [ ] 1.5 实现 `main.rs` 与 `app.rs` 最小可运行框架
...
- [ ] 2. 定义核心数据结构与错误枚举
  - [ ] 2.1 实现 `state/target.rs`：`TargetId`、`TargetKind`、`TargetStatus`、`TargetItem`
    ...
    - _Requirements: 7.5_
    - _Depends on: 1.2_
  - [ ] 2.2 实现 `detector/mod.rs` 中的 `ProcessRecord` 与 `AppType`
    ...
    - _Depends on: 1.2_
  - [ ] 2.3 实现 `state/app_state.rs`：`AppState`、`PrivilegeLevel`、`UiToast`
    ...
    - _Depends on: 2.1_
  - [ ] 2.4 实现 `error.rs`：`AppError`、`TerminateError`、`ScanFailure`、`RmError`
...
- [ ] 3. 实现系统进程分类器与文件夹枚举工具
  - [ ] 3.1 实现 `sys_classifier/blacklist.rs`：硬编码黑名单常量
    ...
    - _Requirements: 5.1_
    - _Depends on: 1.2_
  - [ ] 3.2 实现 `sys_classifier::is_system_process(rec: &PartialRecord) -> bool`
  ...
  - [ ] 3.3 Property 8 PBT：系统进程判定算法
    ...
    - _Depends on: 3.2_
  - [ ] 3.4 实现 `enumerate_direct_children(root: &Path) -> Vec<PathBuf>`
    - 返回 `[root]` ∪ `root` 的直接子文件 ∪ 直接子文件夹（深度恰好 = 1），不递归
    - 处理 `read_dir` 失败：吞掉具体子项错误并记录日志，返回至少包含 `root` 的列表
    - _Requirements: 1.7_
    - _Depends on: 1.2_
  - [ ] 3.5 Property 2 PBT：文件夹仅枚举直接子项
...
- [ ] 4. 封装 Restart Manager 调用并定义 trait
  - [ ] 4.1 定义 `detector/restart_manager.rs` 中的 `RestartManagerApi` trait 与 DTO
    ...
    - _Requirements: 9.2_
    - _Depends on: 2.4_
  - [ ] 4.2 实现真实 `Win32RestartManager`：RmStartSession + RmRegisterResources + RmGetList + RmEndSession 的安全包装
...
- [ ] 5. 实现进程详细信息查询模块
  - [ ] 5.1 实现 `process_info::snapshot(pid: u32) -> Result<ProcessSnapshot, AppError>`
  ...
  - [ ] 5.2 实现 `process_info/token.rs`：进程令牌 SID 与账户名查询
    ...
    - _Requirements: 5.2_
    - _Depends on: 5.1_
  - [ ] 5.3 ProcessInfoProvider 集成测试（标记 ignored，由开发者本地手动跑）
...
- [ ] 6. 抽象进程终止接口
  - [ ] 6.1 定义 `terminator.rs` 中的 `ProcessControlApi` trait
    ...
    - _Requirements: 4.3_
    - _Depends on: 2.4_
  - [ ] 6.2 实现 `Win32ProcessControl` 真实实现
    ...
    - _Depends on: 6.1_
  - [ ] 6.3 实现 `MockProcessControl` 测试桩
    - 可脚本化每次 `open_terminate` / `terminate` 的返回值，并记录调用序列
    - 用于 Property 7 与 Property 9 的注入测试
    - _Requirements: 4.5, 4.6, 4.7_
    - _Depends on: 6.1_
  - [ ] 6.4 实现 `force_terminate(pid, expected_start_time, &dyn ProcessControlApi) -> Result<(), TerminateError>`
  ...
  - [ ] 6.4b 实现 `force_terminate_with_timeout` 包装：在专用线程中调用 `force_terminate`，主线程通过 `crossbeam_channel::recv_timeout(5s)` 等待结果
    - 超时返回 `TerminateError::Timeout`；spawn 出去的线程不被强制中断（自然走完）
    - 调用期间 UI 显示"正在结束进程..."状态（需求 4.3）
    - _Requirements: 4.3, 4.4_
    - _Depends on: 6.4_
  - [ ] 6.5 Property 7 PBT：终止操作的错误码映射
  ...
  - [ ] 6.6 系统进程保护单元测试（Property 9 EXAMPLE 部分）
...
- [ ] 8. 整合 Detector：调度 RM + ProcessInfo + Classifier
  - [ ] 8.1 实现 `detector::scan(target: &TargetItem, rm: &dyn RestartManagerApi) -> Result<Vec<ProcessRecord>, ScanFailure>`
  ...
  - [ ] 8.2 实现 `merge_process_records(raw: Vec<RawRecord>, registered_paths: &[PathBuf]) -> Vec<ProcessRecord>`
    - 同一 `pid` 多条记录合并为一条 `ProcessRecord`，`locked_subitem_count` 累加
    - `locked_subpath` 仅在文件夹模式下填，且必须 ∈ `registered_paths`；多个命中时取首个
    - 输出按 `pid` 排序，确保稳定
    - _Requirements: 2.3, 2.4_
    - _Depends on: 2.2_
  - [ ] 8.3 Property 3 PBT：ProcessRecord 合并不变量
...
- [ ] 9. 实现后台轮询调度器
  - [ ] 9.1 定义 `monitor::MonitorCmd` 与 `monitor::ScanEvent` 枚举
  ...
  - [ ] 9.2 实现 `monitor::scheduler::SchedulerState`：纯逻辑状态机
    ...
    - _Depends on: 9.1_
  - [ ] 9.3 实现 `MonitorEngine`：把 SchedulerState 包装成线程化运行体
  ...
  - [ ] 9.5 Property 6 PBT：同 target 不并发扫描
...
- [ ] 10. 实现状态机 reducer 与目标添加流程
  - [ ] 10.1 定义 `state::UiCmd` 枚举
    ...
    - _Depends on: 2.3_
  - [ ] 10.2 实现纯函数 `apply(state: &mut AppState, cmd: UiCmd)` 处理 UI 命令
  ...
  - [ ] 10.3 实现 `apply_scan_event(state: &mut AppState, ev: ScanEvent)` 把后台事件应用到 state
  ...
  - [ ] 10.4 实现 `try_add_target(state: &mut AppState, path: PathBuf, fs: &dyn FsProbe) -> Result<TargetId, AddError>`
    ...
    - _Depends on: 2.3_
  - [ ] 10.5 Property 1 PBT：try_add_target 的输入归并语义
    ...
    - _Depends on: 10.4_
  - [ ] 10.6 Property 5 PBT：AppState 反映最新扫描结果
    ...
    - _Depends on: 10.3_
  - [ ] 10.7 Property 10 PBT：列表移除/清空的最终状态
    ...
    - _Depends on: 10.2_
  - [ ] 10.8 Property 12 PBT：取消操作的状态恒等
    ...
    - _Depends on: 10.2_
  - [ ] 10.9 实现 `BatchAddSummary` 与批量添加聚合 Toast
    ...
    - _Depends on: 10.4_
  - [ ] 10.10 Property 13 PBT：批量添加结果的聚合
...
- [ ] 12. 实现 UI 主框架
  - [ ] 12.1 在 `app.rs` 中接入 `Arc<Mutex<AppState>>` + `Sender<MonitorCmd>` + `Receiver<ScanEvent>`
  ...
  - [ ] 12.3 实现顶部工具栏 `egui::TopBottomPanel::top`
    ...
    - _Depends on: 12.1_
  - [ ] 12.4 实现 `ui/target_list.rs`：目标项列表与状态展示
    ...
    - _Depends on: 12.1_
  - [ ] 12.5 实现 `ui/process_row.rs`：单条 ProcessRecord 渲染
...
- [ ] 13. 实现对话框与系统进程处置建议
  - [ ] 13.1 实现 `ui/dialogs.rs`：二次确认对话框（终止 / 清空列表）
    ...
    - _Depends on: 12.1_
  - [ ] 13.2 接入终止流程：UI 触发 → `force_terminate_with_timeout` → 结果分支
...
- [ ] 14. 实现状态栏与提权按钮
  - [ ] 14.1 实现 `elevation::is_elevated()`
    - `OpenProcessToken` + `GetTokenInformation(TokenElevation)`
    - _Requirements: 6.1_
    - _Depends on: 2.4_
  - [ ] 14.2 实现 `elevation::restart_as_admin()`
...
- [ ] 15. 实现文件/文件夹对话框（rfd 集成）
  - [ ] 15.1 `+ 添加文件` 按钮处理：`rfd::FileDialog::new().pick_files()` 返回多选 `Vec<PathBuf>`，派发 `UiCmd::AddPaths`
    - _Requirements: 1.1_
    - _Depends on: 12.3, 10.4_
  - [ ] 15.2 `+ 添加文件夹` 按钮处理：`rfd::FileDialog::new().pick_folders()` 返回多选，派发 `UiCmd::AddPaths`
    - _Requirements: 1.2_
    - _Depends on: 12.3, 10.4_
...
- [ ] 16. 配置持久化与日志入口
  - [ ] 16.1 实现 `config.rs`：`AppConfig` + 读写
...
- [ ] 17. 集成测试与端到端验证
  - [ ] 17.1 集成测试：真实 tempdir + 持有句柄的辅助进程
